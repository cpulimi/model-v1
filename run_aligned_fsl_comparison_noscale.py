...
def estimate_objective_scale(batch_df: pd.DataFrame, data: dict[str, Any]) -> float:
    # NO-SCALE VARIANT: objective-scale normalization disabled on purpose.
    # Returning 1.0 makes scaled_value = 1.0 * constraint_multiplier (~5), so the
    # min_penalty floor wins in penalty_weights() and penalties equal the BASE
    # penalty (e.g. --min-penalty 50000) instead of being lifted into the millions.
    # This is the only difference from run_aligned_fsl_comparison.py.
    return 1.0
