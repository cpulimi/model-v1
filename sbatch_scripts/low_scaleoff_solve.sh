...
# reads 10 / sweeps 150 / 2 stages (scale-comparison OFF arm, --disable-objective-scale).
...
# dir (results/low_scale_compare/scale_off) so the earlier results/low_std_comparison,
...
# identical here and in low_scaleoff_merge.sh so batch ids and seeds line up.
# Scale-comparison OFF arm: objective_scale OFF via --disable-objective-scale (penalty = 50k floor). SA, reads 10 / sweeps 150 / 2 stages.
# Writes to results/low_scale_compare/scale_off (run-name low_scaleoff) so the earlier
...
python run_parallel_batches.py \
  ...
  --run-name low_scaleoff \
  --output-dir results/low_scale_compare/scale_off \
  --seed 42 \
...
RUN_DIR="results/low_scale_compare/scale_off/low_scaleoff"
SCALE="low"
mkdir -p "$RUN_DIR"
