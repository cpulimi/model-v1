#!/bin/bash
# Parallel-batch SOLVE job for instances_low using the SQA sampler at the
# STANDARD budget (reads 100 / sweeps 3000 / stages 3).
# One array task per batch. --array is set by low_par_sqa_std_launch.sh.
#
# This is the SQA half of a matched-budget SA-vs-SQA comparison. Batching,
# penalties, reads, sweeps, and stages are IDENTICAL to low_par_std_solve.sh --
# the ONLY difference is the sampler backend (SQASampler vs SASampler).
...
# allocated node) and a longer wall clock than the SA half. TEST ON LOW FIRST.
#SBATCH -p general
#SBATCH -q private
#SBATCH -c 128
#SBATCH --mem 0
...
# identical here and in low_par_sqa_std_merge.sh so batch ids and seeds line up.
# STANDARD budget: num-reads 100, num-sweeps 3000, max-stages 3 -- these MUST
# match low_par_std_solve.sh (SA) exactly so the only variable is the sampler.
# --sqa-gamma is set EXPLICITLY so the transverse field is reportable rather
# than the OpenJij library default.
python run_parallel_batches.py \
  --mode solve \
  --batch-id ${SLURM_ARRAY_TASK_ID} \
  --dataset-dir instances_low \
  --run-name low_par_std_sqa \
...
RUN_DIR="results/low_std_comparison/low_par_std_sqa"
SCALE="low"
mkdir -p "$RUN_DIR"
