#!/bin/bash
# One-shot launcher for the SQA half of the matched-budget SA-vs-SQA comparison
...
#     bash sbatch_scripts/low_par_sqa_std_launch.sh
#
...
#
# All solve/merge flags live in low_par_sqa_std_solve.sh and
# low_par_sqa_std_merge.sh; keep them identical there (and matched to the SA
# std scripts except for the sampler) so batch ids, seeds, and decomposition line up.
set -euo pipefail

source "$(dirname "${BASH_SOURCE[0]}")/common.sh"
DATASET=instances_low
RUN_NAME=low_par_std_sqa
OUTDIR=results/low_std_comparison

# These split flags MUST match the batching flags inside low_par_sqa_std_solve.sh.
...
SOLVE_ID=$(sbatch --parsable --array=1-"$N" "$PROJECT/sbatch_scripts/low_par_sqa_std_solve.sh")
echo ">>> solve array submitted: job $SOLVE_ID (tasks 1-$N, side by side)"

# --- 3) submit dependent merge -------------------------------------------
MERGE_ID=$(sbatch --parsable --dependency=afterok:"$SOLVE_ID" "$PROJECT/sbatch_scripts/low_par_sqa_std_merge.sh")
echo ">>> merge submitted: job $MERGE_ID (runs after $SOLVE_ID succeeds)"

echo
echo "Queue:  squeue -u \$USER"
echo "Logs:   logs/low_par_sqa_std_solve_${SOLVE_ID}_<batchid>.out  and  logs/low_par_sqa_std_merge_${MERGE_ID}.out"
