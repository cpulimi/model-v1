#!/bin/bash
# One-shot launcher for ADAPTIVE PARALLEL BATCH (Option 3) on instances_high.
#
# Run this ON THE LOGIN NODE (it submits jobs, it is not itself an sbatch job):
#     bash sbatch_scripts/high_par_launch.sh
#
...
#
# All solve/merge flags live in high_par_solve.sh and high_par_merge.sh; keep
...
RUN_NAME=high_par
OUTDIR=results/high_adaptive_parallel

# These split flags MUST match the batching flags inside high_par_solve.sh.
...
SOLVE_ID=$(sbatch --parsable --array=1-"$N" "$PROJECT/sbatch_scripts/high_par_solve.sh")
echo ">>> solve array submitted: job $SOLVE_ID (tasks 1-$N, side by side)"

# --- 3) submit dependent merge -------------------------------------------
MERGE_ID=$(sbatch --parsable --dependency=afterok:"$SOLVE_ID" "$PROJECT/sbatch_scripts/high_par_merge.sh")
echo ">>> merge submitted: job $MERGE_ID (runs after $SOLVE_ID succeeds)"

echo
echo "Queue:  squeue -u \$USER"
echo "Logs:   logs/high_par_solve_${SOLVE_ID}_<batchid>.out  and  logs/high_par_merge_${MERGE_ID}.out"
