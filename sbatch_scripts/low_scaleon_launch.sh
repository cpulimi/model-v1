#!/bin/bash
# One-shot launcher: objective_scale ON arm on FULL instances_low (200 hubs, 600 parts).
# SA sampler, reads 10 / sweeps 150 / 2 stages. Pair with low_scaleoff_launch.sh.
# Writes to the NEW results/low_scale_compare/scale_on folder so earlier runs (low_std_comparison,
...
#
# All solve/merge flags live in low_scaleon_solve.sh and low_scaleon_merge.sh;
# keep them identical there so batch ids, seeds, and the decomposition match.
set -euo pipefail

source "$(dirname "${BASH_SOURCE[0]}")/common.sh"
DATASET=instances_low
RUN_NAME=low_scaleon
OUTDIR=results/low_scale_compare/scale_on

# These split flags MUST match the batching flags inside low_scaleon_solve.sh.
...
SOLVE_ID=$(sbatch --parsable --array=1-"$N" "$PROJECT/sbatch_scripts/low_scaleon_solve.sh")
echo ">>> solve array submitted: job $SOLVE_ID (tasks 1-$N, side by side)"

# --- 3) submit dependent merge -------------------------------------------
MERGE_ID=$(sbatch --parsable --dependency=afterok:"$SOLVE_ID" "$PROJECT/sbatch_scripts/low_scaleon_merge.sh")
echo ">>> merge submitted: job $MERGE_ID (runs after $SOLVE_ID succeeds)"

echo
echo "Queue:  squeue -u \$USER"
echo "Logs:   logs/low_scaleon_solve_${SOLVE_ID}_<batchid>.out  and  logs/low_scaleon_merge_${MERGE_ID}.out"
