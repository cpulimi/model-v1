#!/bin/bash
# MAX-RESOURCE one-shot launcher (Option 3) for instances_medium.
# Heavy SA budget (100 reads / 3000 sweeps / 3 stages) on exclusive nodes.
# Run ON THE LOGIN NODE:  bash sbatch_scripts/med_par_heavy_launch.sh
set -euo pipefail

source "$(dirname "${BASH_SOURCE[0]}")/common.sh"
DATASET=instances_medium
RUN_NAME=med_par_heavy
OUTDIR=results/medium_adaptive_parallel_heavy

# MUST match the batching flags inside med_par_heavy_solve.sh.
PART_BATCH_SIZE=1000
MAX_Z=600000

cd "$PROJECT"
mkdir -p "$PROJECT/logs"
echo ">>> project root: $PROJECT"

module purge
module load mamba/latest gurobi/13.0.1
set +u
if [[ -f "$HOME/.bashrc" ]]; then source "$HOME/.bashrc"; fi
source activate qubo 2>/dev/null || conda activate qubo 2>/dev/null || mamba activate qubo
set -u
export PYTHONHASHSEED=0

echo ">>> splitting into batches ..."
python run_parallel_batches.py --mode split \
  --dataset-dir "$DATASET" --run-name "$RUN_NAME" --output-dir "$OUTDIR" \
  --part-batch-size "$PART_BATCH_SIZE" --max-z-vars-per-batch "$MAX_Z"

MANIFEST="$OUTDIR/$RUN_NAME/parallel_work/batches_manifest.json"
N=$(python -c "import json; print(json.load(open('$MANIFEST'))['total_batches'])")
if [[ -z "$N" || "$N" -lt 1 ]]; then
  echo "ERROR: could not determine batch count from $MANIFEST" >&2
  exit 1
fi
echo ">>> discovered $N batches (will run side by side as array 1-$N)"
echo ">>> SA budget: 100 reads / 3000 sweeps / 3 stages (est. ~28-30h wall per job)"

SOLVE_ID=$(sbatch --parsable --array=1-"$N" "$PROJECT/sbatch_scripts/med_par_heavy_solve.sh")
echo ">>> solve array submitted: job $SOLVE_ID (tasks 1-$N, side by side)"

MERGE_ID=$(sbatch --parsable --dependency=afterok:"$SOLVE_ID" "$PROJECT/sbatch_scripts/med_par_heavy_merge.sh")
echo ">>> merge submitted: job $MERGE_ID (runs after $SOLVE_ID succeeds)"

echo
echo "Queue:  squeue -u \$USER"
echo "Logs:   logs/med_par_heavy_solve_${SOLVE_ID}_<batchid>.out  and  logs/med_par_heavy_merge_${MERGE_ID}.out"
echo "Final outputs: $OUTDIR/$RUN_NAME/qubo/  (summary.json, batch_summary.csv, ...)"
