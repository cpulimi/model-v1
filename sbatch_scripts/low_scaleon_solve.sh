...
# reads 10 / sweeps 150 / 2 stages (scale-comparison ON arm, objective_scale ON).
...
# dir (results/low_scale_compare/scale_on) so the earlier results/low_std_comparison,
...
# identical here and in low_scaleon_merge.sh so batch ids and seeds line up.
# Scale-comparison ON arm: objective_scale ON (penalty ~2.5M). SA, reads 10 / sweeps 150 / 2 stages.
# Writes to results/low_scale_compare/scale_on (run-name low_scaleon) so the earlier
...
python run_parallel_batches.py \
  ...
  --run-name low_scaleon \
  --output-dir results/low_scale_compare/scale_on \
  --seed 42 \
...
RUN_DIR="results/low_scale_compare/scale_on/low_scaleon"
SCALE="low"
mkdir -p "$RUN_DIR"
