...
# One array task per batch (--array set by med_par_max_launch.sh).
...
python run_parallel_batches.py \
  ...
  --run-name med_par_max \
  --output-dir results/medium_adaptive_parallel_max \
  --seed 42 \
