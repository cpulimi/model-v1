#!/bin/bash
# One-shot launcher: objective_scale OFF arm (--disable-objective-scale) on FULL instances_low.
# SA sampler, reads 10 / sweeps 150 / 2 stages. Pair with low_scaleon_launch.sh.
# Writes to the NEW results/low_scale_compare/scale_off folder so earlier runs (low_std_comparison,
...
#
# All solve/merge flags live in low_scaleoff_solve.sh and low_scaleoff_merge.sh;
# keep them identical there so batch ids, seeds, and the decomposition match.
set -euo pipefail

source "$(dirname "${BASH_SOURCE[0]}")/common.sh"
DATASET=instances_low
RUN_NAME=low_scaleoff
OUTDIR=results/low_scale_compare/scale_off

# These split flags MUST match the batching flags inside low_scaleoff_solve.sh.
...
SOLVE_ID=$(sbatch --parsable --array=1-"$N" "$PROJECT/sbatch_scripts/low_scaleoff_solve.sh")
echo ">>> solve array submitted: job $SOLVE_ID (tasks 1-$N, side by side)"

# --- 3) submit dependent merge -------------------------------------------
MERGE_ID=$(sbatch --parsable --dependency=afterok:"$SOLVE_ID" "$PROJECT/sbatch_scripts/low_scaleoff_merge.sh")
echo ">>> merge submitted: job $MERGE_ID (runs after $SOLVE_ID succeeds)"

echo
echo "Queue:  squeue -u \$USER"
echo "Logs:   logs/low_scaleoff_solve_${SOLVE_ID}_<batchid>.out  and  logs/low_scaleoff_merge_${MERGE_ID}.out"
